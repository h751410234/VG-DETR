...
def get_pseudo_label_via_threshold(results,threshold = 0.8):
    ...
    for n,result in enumerate(results):  #每张图的预测结果
        ...
        vaild_logits = result['logits'][vaild_idx]
        if len(vaild_labels) > 0 :
            ...
            cache_logits_dict[n] = vaild_logits
    return cache_idx_list,cache_labels_dict,cache_boxes_dict,cache_scores_dict,cache_logits_dict

def deal_pesudo_label(unlabel_target_list,idx_list,pesudo_labels_dict,pesudo_boxes_dict,scores_dcit,logits_dict):
    ...
    for i in idx_list:
        ...
        cache_unlabel_target_format['logits'] = logits_dict[i]
...
def rescale_pseudo_targets(unlabel_samples_img,unlabel_pseudo_targets,nms_th = 0.7):
    ...
    for k,v in unlabel_pseudo_targets.items():
        ...
        keep_inds = batched_nms(unlabel_pseudo_targets[k]['boxes'],
                                ...
                                unlabel_pseudo_targets[k]['labels'], nms_th)
        ...
        unlabel_pseudo_targets[k]['logits'] = unlabel_pseudo_targets[k]['logits'][keep_inds]
        #（3）比例缩放
...
def draw_img(img,unlabel_samples_img_strong_aug,data_dict,unlabel_target,save_dir,_idx):
  ...
  #      cv2.imwrite(os.path.join(save_dir,'%s.jpg'%_idx),img2)
    ...
    if unlabel_samples_img_strong_aug is not None:
        ...
        for i, box in enumerate(boxes):
            ...
            # 获取边界框坐标
            x_c, y_c, w, h = [int(val) for val in box]
            x1, y1 = x_c - w // 2, y_c - h // 2
            x2, y2 = x_c + w // 2, y_c + h // 2

            # 在原图和增强图上画框
            img = cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
            unlabel_samples_img_strong_aug = cv2.rectangle(unlabel_samples_img_strong_aug, (x1, y1), (x2, y2), color, 2)
       # cv2.imshow('a',img)
       # cv2.imshow('b',unlabel_samples_img_strong_aug)
        cv2.imwrite(os.path.join(save_dir,'%s.jpg'%_idx),img)
...
def show_pesudo_label_with_gt(unlabel_img_array,unlabel_pseudo_targets,unlabel_targets,idx_list,unlabel_samples_img_strong_aug_array,_idx,save_dir = './show_pseudo_VPM_GMM5'):
    _make_dir(save_dir)
    ...
    for n,idx in enumerate(idx_list):
        ...
        draw_img(unlabel_img,unlabel_samples_img_strong_aug,unlabel_pseudo_target,unlabel_target,save_dir,_idx)
