...
#------自训练增添--------
from engine import train_one_epoch_with_self_training
...
def build_teacher(args, student_model):
    teacher_model,_,_ = build_model_main(args)
...
def main(args):
    ...
    selected_indices_npy_path = args.labeled_data_idx_numpy
    ...
    subset_remaining = Subset(dataset_train_strong_aug, remaining_indices)

    # -----------------------------------------------------------

    sampler_train_ordered = torch.utils.data.RandomSampler(subset_ordered)
    sampler_train_remaining = torch.utils.data.RandomSampler(subset_remaining)
    sampler_val = torch.utils.data.SequentialSampler(dataset_val)

    batch_sampler_train_ordered = torch.utils.data.BatchSampler(
        sampler_train_ordered, args.batch_size, drop_last=True)

    batch_sampler_train_remaining = torch.utils.data.BatchSampler(
        sampler_train_remaining, args.batch_size, drop_last=True)
    ...
    data_loader_train_remaining = DataLoader(subset_remaining, batch_sampler=batch_sampler_train_remaining,
                                   collate_fn=utils.collate_fn_self_training, num_workers=args.num_workers)
    ...
    if args.onecyclelr:
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, steps_per_epoch=len(data_loader_train_remaining), epochs=args.epochs, pct_start=0.2)
    ...
    # ==========self-training准备工作=======================
    #1.创建学生模型加载预训练权重
    checkpoint = torch.load(args.source_trained_model, map_location='cpu')
    ...
    #2.============创建Teacher模型=====================
    # Build teacher model
    model_tch = build_teacher(args, model_without_ddp).to(device)

    # student model(学生模型) 指标记录
    ...
    # ===============自适应阈值 20241223=================
    th = args.pseudo_label_threshold
    AdaThresh = None
    if args.adaptive_thoushold:
        from models.dino.pseudo_label_weights import AdaptiveThreshold
        # 自适应阈值网络初始化
        AdaThresh = AdaptiveThreshold(args.num_classes , th)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_with_self_training(
            model, model_tch, criterion, data_loader_train_ordered, data_loader_train_remaining, optimizer, device,
            epoch,
            args.clip_max_norm, wo_class_error=wo_class_error, lr_scheduler=lr_scheduler, args=args,
            logger=(logger if args.save_log else None), ema_m=ema_m,pseudo_weight = None, AdaTh = AdaThresh)
        ...
        results_student = []
        coco_eval = coco_evaluator.coco_eval['bbox']  # 假设你只看 bbox 的评估结果
        precision_data = coco_eval.eval["precision"]  # 其 shape 通常是 [T, R, K, A, M]

        for idx, cat_id in enumerate(coco_eval.params.catIds):
            # 第一个维度=0 表示 IoU=0.5
            # 第二个维度 : 表示遍历所有 recall 门槛
            # 第三个维度=idx 表示当前类别
            # 第四个维度=0 表示所有面积范围(all)
            # 第五个维度=-1 表示使用最后一个 maxDet（通常是 100 或者更大的那个）
            precision = precision_data[0, :, idx, 0, -1]
            # 去掉无效值后取平均
            ap = np.mean(precision[precision > -1])
            results_student.append((cat_id, ap))

        result_str = f"epoch {epoch + 1}: " + ", ".join([f"cat_{cat_id}={ap:.4f}" for cat_id, ap in results_student])
        with open(output_dir / "results_student.txt", "a", encoding="utf-8") as f:
            f.write(result_str + "\n")
        ...
        test_stats_best_ema, coco_evaluator_best_ema = evaluate(
            model_tch, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir,
            wo_class_error=wo_class_error, args=args, logger=(logger if args.save_log else None)
        )

        results_teacher = []
        coco_eval_t = coco_evaluator_best_ema.coco_eval['bbox']  # 假设你只看 bbox 的评估结果
        precision_data_t = coco_eval_t.eval["precision"]  # 其 shape 通常是 [T, R, K, A, M]

        for idx, cat_id in enumerate(coco_eval_t.params.catIds):
            # 第一个维度=0 表示 IoU=0.5
            # 第二个维度 : 表示遍历所有 recall 门槛
            # 第三个维度=idx 表示当前类别
            # 第四个维度=0 表示所有面积范围(all)
            # 第五个维度=-1 表示使用最后一个 maxDet（通常是 100 或者更大的那个）
            precision_t = precision_data_t[0, :, idx, 0, -1]
            # 去掉无效值后取平均
            ap_t = np.mean(precision_t[precision_t > -1])
            results_teacher.append((cat_id, ap_t))

        result_str_t = f"epoch {epoch + 1}: " + ", ".join([f"cat_{cat_id}={ap_t:.4f}" for cat_id, ap_t in results_teacher])
        with open(output_dir / "results_teacher.txt", "a", encoding="utf-8") as f:
            f.write(result_str_t + "\n")
        ...
        if args.output_dir and utils.is_main_process():
            ...
            if epoch >= args.burn_epochs:
                ...
                if test_stats_best_ema['coco_eval_bbox'][1] >= best_ema_teacher_fitness:
                    best_ema_teacher_fitness = test_stats_best_ema['coco_eval_bbox'][1]
                    checkpoint_path = output_dir / 'best_ema_model.pth'
                    cache_best_ema_model_epoch = epoch  # 记录epoch数
                    utils.save_on_master({
                        'ema_model': model_tch.state_dict(),
                        'epoch': epoch,
                    }, checkpoint_path)
            ...
            with open(output_dir / "log_best.txt", 'w') as f:
                ...
                f.write(
                    'best_teacher -->  map50:%s , epoch:%s\n' % (best_ema_teacher_fitness, cache_best_ema_model_epoch))
