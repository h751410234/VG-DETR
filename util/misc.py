...
#---增添self_training
def collate_fn_self_training(batch):

    imgs, labels,imgs_strong_aug,img_no_normalize_weak,img_no_normalize_strong, flipped_flags = list(zip(*batch))
    samples = nested_tensor_from_tensor_list(imgs)

    #判断是否经过强增广处理
    if imgs_strong_aug[0] is not None:
        samples_strong_aug = nested_tensor_from_tensor_list(imgs_strong_aug)
    else:
        samples_strong_aug = None
    return samples, labels,samples_strong_aug,img_no_normalize_weak,img_no_normalize_strong, flipped_flags
