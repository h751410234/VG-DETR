...
colors = [
    (0.8314, 0.5686, 0.8510),  # 紫  #飞机
    (0.5647, 0.6902, 0.8157),  # 浅蓝  #油罐
    (0.3804, 0.5216, 0.4863),  # 深绿 #船
]
int_colors = []
for color in colors:
...
def visualize_and_save(image, pred_dict, save_path):
    ...
    for box, label in zip(pred_dict['boxes'], pred_dict['box_label']):
        ...
        # else:
        #     draw.text((x1, y1), label_list[label-1], fill='white')
...
model_config_path = "config/DINO_self_training/DINO_4scale.py" # change the path of the model config file
model_checkpoint_path = "best_ema_model.pth" # change the path of the model checkpoint
...
# transform images
transform = T.Compose([
    T.RandomResize([800], max_size=800),    T.ToTensor(),
...
if __name__ == '__main__':
    ...
    label_list = ['plane','stroage-tank','ship']
    ...
    for n,img_name in enumerate(fliter_img_list):
        ...
        # predict images
        output,_,_,_= model.cuda()(image[None].cuda())
