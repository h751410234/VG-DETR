...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x: Tensor, num_cls_token, masks=None):
        ...
        x_norm = self.norm(x)
        return {
            "x_norm_clstoken": x_norm[:, 0:num_cls_token],
            # "x_norm_regtokens": x_norm[:, 1 : self.num_register_tokens + 1],
            "x_norm_patchtokens": x_norm[:, num_cls_token:],
            "x_prenorm": x,
        }
    ...
    def forward(self, x, num_cls_token=1, masks=None):
        ...
        return cls_token, patch_tokens.reshape(B, num_patch, num_patch_w, self.embed_dim).permute(0, 3, 1, 2)
